...
func main() {
	...
	r.GET("/sqrt/:x", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"calculation": "sqrt(" + c.Param("x") + ")", "result": math.Sqrt(x)})
	})
	...
	r.GET("/mod/:a/:b", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"calculation": c.Param("a") + "%" + c.Param("b"), "result": math.Mod(a, b)})
	})
	...
	r.GET("/pow/:a/:b", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"calculation": c.Param("a") + "^" + c.Param("b"), "result": math.Pow(a, b)})
	})
	...
	r.GET("/log/:x", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"calculation": "log(" + c.Param("x") + ")", "result": math.Log(x)})
	})
